/*
    This file was generated automatically by Alchitry Labs 2.0.30-BETA.
    Do not edit this file directly. Instead edit the original Lucid source.
    This is a temporary file and any changes made to it will be destroyed.
*/

module manual_tester (
        input wire clk,
        input wire rst,
        input wire [0:0] select_button,
        input wire [0:0] start_button,
        input wire [3:0] row,
        output reg [3:0] col,
        output reg [15:0] led_grid,
        output reg [15:0] ledout,
        output reg [6:0] io_segment,
        output reg [3:0] seg_selector
    );
    localparam E_States_IDLE = 5'h0;
    localparam E_States_SELECT_GAME = 5'h1;
    localparam E_States_LOAD = 5'h2;
    localparam E_States_LOAD_INCREASE = 5'h3;
    localparam E_States_LOAD_ADVANCE = 5'h4;
    localparam E_States_LOAD_CHECK = 5'h5;
    localparam E_States_SELECT_LEVEL = 5'h6;
    localparam E_States_LOAD_MAP_TO_3 = 5'h7;
    localparam E_States_LOAD_MAP_TO_4 = 5'h8;
    localparam E_States_PLAYER_INPUT = 5'h9;
    localparam E_States_RESET = 5'ha;
    localparam E_States_UPDATE = 5'hb;
    localparam E_States_GAMEOVER = 5'hc;
    localparam E_States_CHECK = 5'hd;
    localparam E_States_LOAD_IN = 5'he;
    localparam E_States_LOAD_COUNTER = 5'hf;
    localparam E_States_LEVEL_ADVANCE = 5'h10;
    localparam E_States_CHECK_LEVEL = 5'h11;
    logic [3:0] M_button_map_col;
    logic [3:0] M_button_map_row;
    logic [7:0] M_button_map_out;
    
    button_map button_map (
        .rst(rst),
        .clk(clk),
        .col(M_button_map_col),
        .row(M_button_map_row),
        .out(M_button_map_out)
    );
    
    
    logic [7:0] D_input_buffer_d, D_input_buffer_q = 0;
    logic D_gamemode_d, D_gamemode_q = 0;
    logic [4:0] D_state_d, D_state_q = 5'h0;
    logic [7:0] D_led_d, D_led_q = 0;
    logic [4:0] M_reg_ra1;
    logic [4:0] M_reg_ra2;
    logic [4:0] M_reg_rc;
    logic [31:0] M_reg_rd1;
    logic [31:0] M_reg_rd2;
    logic M_reg_z;
    logic [15:0] M_reg_button_led;
    logic [31:0] M_reg_level_select;
    logic [31:0] M_reg_count_min;
    logic [31:0] M_reg_rom_sel;
    logic [31:0] M_reg_write_data;
    logic M_reg_write_enable;
    logic [15:0] M_reg_button1;
    
    simple_ram L_reg (
        .clk(clk),
        .ra1(M_reg_ra1),
        .ra2(M_reg_ra2),
        .rc(M_reg_rc),
        .rd1(M_reg_rd1),
        .rd2(M_reg_rd2),
        .z(M_reg_z),
        .button_led(M_reg_button_led),
        .level_select(M_reg_level_select),
        .count_min(M_reg_count_min),
        .rom_sel(M_reg_rom_sel),
        .write_data(M_reg_write_data),
        .write_enable(M_reg_write_enable),
        .button1(M_reg_button1)
    );
    
    
    localparam _MP_DIGITS_1676852617 = 3'h4;
    localparam _MP_DIV_1676852617 = 3'h4;
    logic [3:0][3:0] M_seg_values;
    logic [7:0] M_seg_seg;
    logic [3:0] M_seg_sel;
    
    multi_seven_seg #(
        .DIGITS(_MP_DIGITS_1676852617),
        .DIV(_MP_DIV_1676852617)
    ) seg (
        .rst(rst),
        .clk(clk),
        .values(M_seg_values),
        .seg(M_seg_seg),
        .sel(M_seg_sel)
    );
    
    
    localparam _MP_DIGITS_1160655609 = 3'h4;
    localparam _MP_DIV_1160655609 = 3'h4;
    logic [3:0][3:0] M_word_seg_values;
    logic [7:0] M_word_seg_seg;
    logic [3:0] M_word_seg_sel;
    
    multi_word_seg #(
        .DIGITS(_MP_DIGITS_1160655609),
        .DIV(_MP_DIV_1160655609)
    ) word_seg (
        .rst(rst),
        .clk(clk),
        .values(M_word_seg_values),
        .seg(M_word_seg_seg),
        .sel(M_word_seg_sel)
    );
    
    
    logic [31:0] M_alu_a;
    logic [31:0] M_alu_b;
    logic [5:0] M_alu_alufn;
    logic [31:0] M_alu_out;
    logic M_alu_z;
    logic M_alu_v;
    logic M_alu_n;
    
    alu alu (
        .a(M_alu_a),
        .b(M_alu_b),
        .alufn(M_alu_alufn),
        .out(M_alu_out),
        .z(M_alu_z),
        .v(M_alu_v),
        .n(M_alu_n)
    );
    
    
    logic [5:0] M_control_unit_opcode;
    logic [5:0] M_control_unit_alufn;
    logic M_control_unit_bsel;
    logic M_control_unit_we;
    logic [1:0] M_control_unit_pc_sel;
    
    cu control_unit (
        .opcode(M_control_unit_opcode),
        .alufn(M_control_unit_alufn),
        .bsel(M_control_unit_bsel),
        .we(M_control_unit_we),
        .pc_sel(M_control_unit_pc_sel)
    );
    
    
    logic [31:0] M_iROM_address;
    logic [31:0] M_iROM_out;
    
    instruction_ROM iROM (
        .address(M_iROM_address),
        .out(M_iROM_out)
    );
    
    
    localparam _MP_DIGITS_299498829 = 3'h4;
    localparam _MP_LEADING_ZEROS_299498829 = 1'h1;
    logic [13:0] M_decimal_renderer_value;
    logic [3:0][3:0] M_decimal_renderer_digits;
    
    bin_to_dec #(
        .DIGITS(_MP_DIGITS_299498829),
        .LEADING_ZEROS(_MP_LEADING_ZEROS_299498829)
    ) decimal_renderer (
        .value(M_decimal_renderer_value),
        .digits(M_decimal_renderer_digits)
    );
    
    
    always @* begin
        D_led_d = D_led_q;
        D_input_buffer_d = D_input_buffer_q;
        D_state_d = D_state_q;
        D_gamemode_d = D_gamemode_q;
        
        D_led_d = D_led_q;
        ledout = M_reg_z;
        col = M_button_map_col;
        M_button_map_row = row;
        led_grid = M_reg_button_led;
        M_decimal_renderer_value = M_reg_count_min[4'hd:1'h0];
        M_word_seg_values = {{4'hf, 4'hf, 4'hf, 4'hf}};
        M_seg_values = M_decimal_renderer_digits;
        io_segment = ~M_seg_seg;
        seg_selector = ~M_seg_sel;
        M_control_unit_opcode = M_iROM_out[5'h1f:5'h1a];
        M_reg_ra1 = M_iROM_out[5'h14:5'h10];
        M_reg_ra2 = M_iROM_out[4'hf:4'hb];
        M_reg_rc = M_iROM_out[5'h19:5'h15];
        M_reg_write_data = M_alu_out;
        M_reg_write_enable = M_control_unit_we;
        M_alu_alufn = M_control_unit_alufn;
        M_alu_a = M_reg_rd1;
        
        case (M_control_unit_bsel)
            1'h1: begin
                M_alu_b = M_reg_rd2;
            end
            1'h0: begin
                M_alu_b = M_iROM_out[4'hf:1'h0];
            end
            default: begin
                M_alu_b = M_iROM_out[4'hf:1'h0];
            end
        endcase
        D_input_buffer_d = D_input_buffer_q;
        M_iROM_address = D_input_buffer_q;
        D_state_d = D_state_q;
        D_gamemode_d = D_gamemode_q;
        
        case (D_state_q)
            5'h0: begin
                D_state_d = 5'h1;
            end
            5'h1: begin
                M_word_seg_values = {{{4'hf}}, {{4'hf}}, {{4'hf}}, {{3'h0, D_gamemode_q}}};
                io_segment = ~M_word_seg_seg;
                seg_selector = ~M_word_seg_sel;
                if (select_button) begin
                    D_gamemode_d = D_gamemode_q ^ 1'h1;
                end
                if (start_button) begin
                    M_iROM_address = D_gamemode_q;
                    D_state_d = 5'h2;
                end
            end
            5'h2: begin
                M_iROM_address = M_reg_rom_sel;
                D_state_d = 5'h3;
            end
            5'h3: begin
                M_iROM_address = 6'h24;
                D_state_d = 5'h5;
            end
            5'h5: begin
                M_iROM_address = 6'h25;
                D_state_d = 5'h4;
            end
            5'h4: begin
                if (~M_reg_z) begin
                    D_state_d = 5'h6;
                    M_iROM_address = 6'h27;
                end else begin
                    M_iROM_address = 2'h2;
                    D_state_d = 5'h2;
                end
            end
            5'h6: begin
                M_iROM_address = 7'h41;
                M_seg_values = {{{4'hf}}, {{4'hf}}, {M_reg_level_select[3'h7:3'h4]}, {M_reg_level_select[2'h3:1'h0]}};
                D_led_d = M_reg_z;
                if (select_button) begin
                    M_iROM_address = 6'h3f;
                    D_state_d = 5'h10;
                end
                if (start_button) begin
                    M_iROM_address = 7'h40;
                end
            end
            5'h10: begin
                D_led_d = M_reg_z;
                M_iROM_address = 2'h2;
                if (~M_reg_z) begin
                    M_iROM_address = 7'h40;
                end
                D_state_d = 5'h6;
            end
            5'h7: begin
                M_iROM_address = 6'h29;
                D_state_d = 5'h8;
            end
            5'h8: begin
                M_iROM_address = 6'h2c;
                D_state_d = 5'hf;
            end
            5'hf: begin
                M_iROM_address = 6'h3d;
                D_state_d = 5'h9;
            end
            5'h9: begin
                ledout = M_reg_count_min[3'h7:1'h0];
                if ((|M_button_map_out[2'h3:1'h0])) begin
                    D_state_d = 5'hb;
                end
                
                case (M_button_map_out)
                    8'h11: begin
                        M_iROM_address = 6'h2d;
                        D_led_d = 6'h2d;
                    end
                    8'h21: begin
                        M_iROM_address = 6'h2e;
                        D_led_d = 6'h2e;
                    end
                    8'h41: begin
                        M_iROM_address = 6'h2f;
                        D_led_d = 6'h2f;
                    end
                    8'h81: begin
                        M_iROM_address = 6'h30;
                        D_led_d = 6'h30;
                    end
                    8'h12: begin
                        M_iROM_address = 6'h31;
                        D_led_d = 6'h31;
                    end
                    8'h22: begin
                        M_iROM_address = 6'h32;
                        D_led_d = 6'h32;
                    end
                    8'h42: begin
                        M_iROM_address = 6'h33;
                        D_led_d = 6'h33;
                    end
                    8'h82: begin
                        M_iROM_address = 6'h34;
                        D_led_d = 6'h34;
                    end
                    8'h14: begin
                        M_iROM_address = 6'h35;
                        D_led_d = 6'h35;
                    end
                    8'h24: begin
                        M_iROM_address = 6'h36;
                        D_led_d = 6'h36;
                    end
                    8'h44: begin
                        M_iROM_address = 6'h37;
                        D_led_d = 6'h37;
                    end
                    8'h84: begin
                        M_iROM_address = 6'h38;
                        D_led_d = 6'h38;
                    end
                    8'h18: begin
                        M_iROM_address = 6'h39;
                        D_led_d = 6'h39;
                    end
                    8'h28: begin
                        M_iROM_address = 6'h3a;
                        D_led_d = 6'h3a;
                    end
                    8'h48: begin
                        M_iROM_address = 6'h3b;
                        D_led_d = 6'h3b;
                    end
                    8'h88: begin
                        M_iROM_address = 6'h3c;
                        D_led_d = 6'h3c;
                    end
                    default: begin
                        D_led_d = D_led_q;
                    end
                endcase
            end
            5'hb: begin
                D_led_d = 6'h3e;
                M_iROM_address = 6'h3e;
                D_state_d = 5'h9;
            end
            5'hd: begin
                M_iROM_address = 6'h3e;
            end
        endcase
    end
    
    
    always @(posedge (clk)) begin
        D_input_buffer_q <= D_input_buffer_d;
        D_gamemode_q <= D_gamemode_d;
        D_state_q <= D_state_d;
        D_led_q <= D_led_d;
        
    end
endmodule